...
implementation 'org.apache.ignite:ignite-indexing:${selectedVersion}'`;
  ...
  const renderDownloadContent = () => {
    ...
    const rawUrl = getDownloadUrl(currentRelease, activeMethod);
    if (!rawUrl) return <p>No download available for this version.</p>;

    // 1) resolve placeholder -> preferred mirror
    const resolvedUrl = resolveMirrorUrl(rawUrl, preferredMirror);
    // 2) convert to closer.lua download URL
    const downloadHref = toCloserUrl(resolvedUrl);

    const fileName = getFileName(downloadHref);
        ...
        <div className={styles.downloadContent}>
          ...
          <div className={styles.downloadCard}>
            ...
            <a
                ...
                onClick={() => handleDownloadClick(downloadHref)}>
              Download
            </a>
          </div>
          ...
          <div className={styles.verifyRow}>
            ...
            <a href={withSuffix(downloadHref, '.asc?action=download')} target="_blank" rel="noopener noreferrer">
              PGP Signature
            </a>
            <a
                href={withSuffix(downloadHref, '.sha512?action=download')}
                target="_blank"
                rel="noopener noreferrer">
              SHA512 Checksum
            </a>
