...
#   ./build-local.sh --serve            # Build and start server
#   ./build-local.sh --skip-docs-build  # Skip Docusaurus docs build (use existing)
#   ./build-local.sh --skip-site-build  # Skip website build (use existing)
...
# Step 4: Copy Ignite 3 docs to correct path
echo ""
...
# Step 4: Copy pre-built Ignite 2 docs
echo ""
...
# Step 5: Copy original assets (versioning.js, docs.css, etc.)
echo ""
...
# Step 6: Copy Pagefind search index
echo ""
...
# Step 7: Copy version metadata files
echo ""
...
# Note: We skip copying 'latest' text files here because step 8 creates
...
# Step 8: Create latest version aliases for static server
# (npx serve doesn't process .htaccess, so we copy content to 'latest' directories)
echo ""
...
if [ "$SERVE" = true ]; then
  ...
  echo "  - Docs (redirects):      http://localhost:$PORT/docs/"
